...
export function BedroomGallery({ images }) {
    ...
    <Section spacing="standard" containerWidth="wide" bg="default" className="border-t border-[var(--color-border-soft)]">
      <div className="mb-12 lg:mb-16">
        <SectionHeading
          ...
          description="Explore beautifully styled bedrooms showcasing a harmonious blend of Scandinavian functionality and luxury design."
          align="center"
        />
      </div>
      ...
      <div ref={galleryRef} className="grid grid-cols-12 gap-4 lg:gap-6 auto-rows-[300px] lg:auto-rows-[400px]">
        {images.map((item) => (
          ...
          >
            <Image
              src={item.image}
              alt={item.title || "Bedroom lifestyle inspiration"}
              fill
              className="object-cover transition-transform duration-[2s] ease-[cubic-bezier(0.25,1,0.5,1)] group-hover:scale-105"
              sizes="(max-width: 768px) 100vw, 60vw"
            />
            {/* Subtle soft overlay */}
            <div className="absolute inset-0 bg-black/0 transition-colors duration-700 group-hover:bg-[#3d3833]/10 pointer-events-none" />
