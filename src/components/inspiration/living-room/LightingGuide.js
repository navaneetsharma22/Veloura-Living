    ...
    <Section spacing="spacious" containerWidth="wide" bg="light" className="border-t border-[var(--color-border-soft)]">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <div className="flex flex-col gap-8">
            {guide.layers.map((layer, index) => (
              <div key={index} className="reveal-light-txt flex gap-6">
                <div className="flex-shrink-0 w-12 h-12 rounded-full border border-[var(--color-border-soft)] bg-white flex items-center justify-center font-heading text-lg font-medium text-[var(--text-heading)]">
                  0{index + 1}
                </div>
                <div>
        ...
        <div className="relative w-full aspect-[4/5] rounded-2xl overflow-hidden shadow-xl order-1 lg:order-2">
          <Image
            src={guide.image}
            alt="Lighting layering in a luxury interior"
            fill
            className="object-cover"
            sizes="(max-width: 1024px) 100vw, 50vw"
          />
          {/* Simulate a warm lighting glow over the image */}
          <div className="absolute inset-0 bg-gradient-to-tr from-[#ffe6c9]/10 to-transparent mix-blend-overlay" />
        </div>
